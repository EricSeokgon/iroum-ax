...
- `evidences` 테이블 (`file_content BYTEA` 컬럼) + 마이그레이션 `0002_evidence_tables.sql`
- 단일 엔드포인트 `POST /api/v1/evidences` (생성+버전 통합, SHA-256 단일 패스 스트리밍)
- `EvidenceBlobStore` / `dbBlobStore` 논리 위치 추상화 (외부 의존 0건, 망분리 정합)
