...
Given/When/Then 형식. REQ당 최소 2 AC, REQ-OBS-UBI-001 4개 sub-clause는 각각 dedicated AC 보유 (lessons #2). **Total AC: 24건** (REQ-mapped 21 + Edge/회귀 3 — spec.md §8.1 카운트 표가 단일 진실). Disambiguation: REQ-mapped 21 = UBI 4 + OBS-001 5 + OBS-002 5 + OBS-003 4 + OBS-004 3. v0.1.2(iteration 3)에서 AC-OBS-001-4/5(두 counter 실제 increment 검증 — circular import 회피 DI + workflow 직접 import) 추가로 OBS-001 3→5. D1(MetricsAuthMiddleware)로 AC-OBS-002-1/3/4가 authn/authz 분리 기준 재작성된 것은 v0.1.1 처리(AC ID 개수 5건 불변).
...
### AC-OBS-001-4 (auth_rejections_total 실제 increment via RejectionObserver DI — REQ-OBS-001-S2)

- Given: `internal/auth` 패키지가 `RejectionObserver` interface를 자체 정의하고 `metrics`가 그 구현체를 제공하며 두 패키지가 순환 import 없이 빌드되는 상태. 테스트는 spy `RejectionObserver`(또는 metrics 구현체)를 `auth.New(..., auth.WithRejectionObserver(obs))`로 주입한 `*auth.TokenValidator` 사용
- When: (i) 잘못된 issuer 토큰, (ii) alg/kty mismatch 토큰, (iii) 만료 토큰, (iv) 블랙리스트 jti 토큰으로 각각 `validator.Verify`를 호출(모두 reject)
- Then: 각 호출에서 observer의 `IncAuthRejection`이 정확한 reason(`invalid_issuer` / `alg_mismatch` / `expired` / `blacklist`)으로 1회씩 호출되어 `iroum_ax_auth_rejections_total{reason}`가 해당 reason별 1 증가한다. observer 미주입(nil) 시 `Verify` reject 동작은 기존과 동일(no-op, AUTH-001 backward-compat). `internal/auth`가 `internal/metrics`를 import하지 않음을 import 그래프(`go list -deps`)로 단언 → `auth → metrics → auth` 순환 부재 확인

### AC-OBS-001-5 (workflow_state_transitions_total 실제 increment via 직접 import — REQ-OBS-001-S3)

- Given: `internal/workflow/state_machine.go`가 `internal/metrics`를 직접 import하고 spy/실제 collector가 등록된 상태
- When: `StateMachine.Start`(PENDING→RUNNING), `Complete`(RUNNING→COMPLETED), `Fail`(→FAILED)를 각각 Commit 성공하도록 실행. 추가로 invalid transition으로 Commit 실패하는 케이스 1건 실행
- Then: Commit 성공한 3개 전이에 대해 `iroum_ax_workflow_state_transitions_total{from,to}`가 각 (from,to) 쌍별 1 증가하고, Commit 실패(rollback)한 전이는 카운트되지 않는다. `workflow → metrics` 직접 import가 순환을 만들지 않음을 import 그래프로 단언(observer 패턴 미적용이 정당함을 확인)

---

## REQ-OBS-002 — /metrics Endpoint + RBAC

### AC-OBS-002-1 (admin → 200 exposition via MetricsAuthMiddleware — REQ-OBS-002-E1/E3/S1)

- Given: `cfg.AuthEnabled=true`, `MetricsAuthMiddleware`로 wrapping된 `/metrics` (auth chain 외부 mount, 주입된 `*auth.TokenValidator`가 `iroum-ax:admin` 토큰을 검증 가능), 유효한 `iroum-ax:admin` 토큰
- When: `Authorization: Bearer <admin-token>` 헤더로 `GET /metrics` 요청
- Then: MetricsAuthMiddleware authn 단계가 `validator.Verify` 성공 후 `auth.WithUser` 주입, authz 단계가 `auth.UserFromContext`→`ParseRolesFromScope`→`IsMetricsAuthorized(roles)=true`(RoleAdmin 포함)로 통과 → HTTP 200, `Content-Type: text/plain; version=0.0.4`, body에 `iroum_ax_` prefix 메트릭 exposition이 포함된다
...
### AC-OBS-002-3 (authentication 실패 → 401 — REQ-OBS-002-U1/E3)

- Given: `cfg.AuthEnabled=true`, `MetricsAuthMiddleware`로 wrapping된 `/metrics`
- When: 다음 3개 sub-case로 `GET /metrics` 요청 — (i) Authorization 헤더 없음, (ii) `Authorization: Token xyz`(Bearer scheme 아님), (iii) `Authorization: Bearer <invalid/expired token>` (`validator.Verify` 실패)
- Then: 세 case 모두 MetricsAuthMiddleware authn 단계에서 HTTP 401, body `{"error":{"code":"UNAUTHENTICATED","message":"authentication required for metrics"}}`, inner promhttp handler 미호출(메트릭 exposition 미노출), authz 단계 미진입

### AC-OBS-002-4 (authentication 성공 + authorization 실패: viewer → 403 — REQ-OBS-002-U2/S1)

- Given: `cfg.AuthEnabled=true`, `MetricsAuthMiddleware`로 wrapping된 `/metrics`, 유효한 `iroum-ax:viewer` 토큰(Verify는 성공하나 RoleAdmin 미포함)
- When: `Authorization: Bearer <viewer-token>` 헤더로 `GET /metrics` 요청
- Then: MetricsAuthMiddleware authn 단계 통과(`validator.Verify` 성공 + `WithUser` 주입) 후 authz 단계에서 `IsMetricsAuthorized(roles)=false` → HTTP 403, body `{"error":{"code":"PERMISSION_DENIED","message":"insufficient scope","details":{"required":"read:metrics"}}}`, `iroum_ax_authz_forbidden_total{role="viewer",method="GET /metrics"}`가 1 증가, inner promhttp handler 미호출(exposition 미노출). 이로써 401(authn 실패, AC-OBS-002-3)과 403(authz 실패, 본 AC)이 분리 검증된다

### AC-OBS-002-5 (AuthEnabled=false backward-compat)

- Given: `cfg.AuthEnabled=false`, `MetricsAuthMiddleware(validator, false)`로 wrapping된 `/metrics`
- When: 인증 헤더 없이 `GET /metrics` 요청
- Then: MetricsAuthMiddleware가 `authEnabled=false` 분기로 authn/authz 모두 skip하고 inner promhttp handler 직접 호출 → HTTP 200 + exposition (SERVER-001 backward-compat 정합 — 인증 없이 통과)
...
- [ ] AC-OBS-* 24건 전부 GREEN (REQ-mapped 21: UBI 4 + OBS-001 5 + OBS-002 5 + OBS-003 4 + OBS-004 3; Edge/회귀 3 — spec.md §8.1 단일 진실)
- [ ] `internal/auth`가 `internal/metrics`를 import하지 않음(import 그래프 단언, `auth → metrics → auth` 순환 부재) — RejectionObserver DI 정상
...
- [ ] MX 태그 5종 추가 (registry ANCHOR / MetricsAuthMiddleware ANCHOR+REASON / InstrumentHTTP NOTE / goroutine WARN+REASON / wiring NOTE), `code_comments: ko`
