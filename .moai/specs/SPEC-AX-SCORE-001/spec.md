...
- 0.1.3 (2026-05-19): SYNC — TDD RED-GREEN-REFACTOR + GAN iter2 완료, status: completed 확정. **GAN iter1 FAIL 46.25 → iter2 PASS 85.25** (DC-UBI-002 audit 원자성 단언 추가·DC-UBI-004 CONFIRMED 불변/SupersedeAndReplaceScore 통합·SEC-03 pgtype.Numeric 정밀도 가드 3건 해소). 구현 커버리지 87.2%, 통합 테스트 `ok store 278.986s` 확인. `PgScoreTx` 7 메서드(`InsertScore`, `GetScoreByID`, `GetScoresByEvaluationItem`, `UpdateScore`, `SupersedeAndReplaceScore`, `SumWeightedByEvaluationItem`, `DetermineGrade`) + `Recorder` 확장(`RecordScoreCreated`, `RecordScoreUpdated`) + 에러 센티널 6종 + DB 마이그레이션 `0004_score_tables.sql`(`scores` + `grade_thresholds` 2테이블) 전부 GREEN. HTTP 핸들러 없음(store+audit 계층만) — CHANGELOG 반영 완료. (작성자: ircp)
