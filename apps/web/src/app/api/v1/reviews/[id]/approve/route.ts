// @MX:NOTE: BFF 리뷰 승인 프록시 — admin 전용. ReviewCard의 UNDER_REVIEW 액션 버튼이 호출.
// SPEC-AX-WEB-001 Phase E REQ-WEB-006 POST /api/v1/reviews/{id}/approve 정합.
...
export async function POST(
  ...
  // 본문 — 빈 본문 허용. 객체이면 comment만 화이트리스트.
  const body = await safeReadCommentBody(request);

  const targetUrl = new URL(
    `/api/v1/reviews/${encodeURIComponent(id)}/approve`,
    getBackendBaseUrl(),
  );
 ...
 * 파싱 실패 시에도 200 OK 진입 가능하도록 빈 객체로 fallback.
 */
